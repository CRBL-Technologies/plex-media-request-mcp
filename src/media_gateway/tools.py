...
def season_is_missing(state: Mapping[str, Any]) -> bool:
    """Whether a season is worth requesting.

    A season Sonarr lists with no episodes has not aired, so it is neither held
    nor missing: there is nothing to acquire. Keep this the only definition --
    the gateway, the card and the picker shortcut must agree on it.
    """

    return not state.get("complete") and int(state.get("episodes") or 0) > 0
